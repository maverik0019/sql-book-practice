...
(
        SELECT 
        date_part('year',interval_since_last) * 12 + date_part('year',interval_since_last)
...
;
--Solo un extra 
--extract(epoch from interval_since_last) / 60 / 60 / 24 / 30.4375
...
--
SELECT
  string_agg(distinct product, '|' ORDER BY product) as basket_signature,
  count(*) as customers
FROM purchases
